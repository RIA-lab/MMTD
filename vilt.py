from models import ViltEmailModel
from Email_dataset import EDPDataset, EDPCollatorViLT
...
if __name__ == '__main__':
    for i in range(3, fold):
        name = 'vilt-fold-' + str(i + 1)
        train_df, test_df = split_data()
        train_dataset = EDPDataset('DATA/email_data/pics', train_df)
        test_dataset = EDPDataset('DATA/email_data/pics', test_df)
        model = ViltEmailModel.from_pretrained("dandelin/vilt-b32-mlm")

        args = TrainingArguments(
            output_dir='./output/vilt/checkpoints/fold' + str(i + 1),
            logging_dir='./output/vilt/log',
        ...
        trainer.compute_metrics = EvalMetrics('output/vilt/results', name, True)
        ...
        # save_config(args.to_dict(), os.path.join('./output/vilt/configs', wandb.run.name + '.yaml'))
        # wandb.finish()
