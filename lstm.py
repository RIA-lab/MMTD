...
from Email_dataset import EDPDataset, EDPTextCollator, TokenizerLSTM
...
if __name__ == '__main__':
    for i in range(fold):
        ...
        wandb.run.name = 'lstm-fold-' + str(i + 1)
        ...
        args = TrainingArguments(
            output_dir='./output/lstm/checkpoints/fold' + str(i + 1),
            logging_dir='./output/lstm/log',
        ...
        trainer = Trainer(
            ...
            data_collator=EDPTextCollator(tokenizer=TokenizerLSTM()),
        ...
        trainer.compute_metrics = EvalMetrics('output/lstm/results', wandb.run.name, True)
        ...
        save_config(args.to_dict(), os.path.join('./output/lstm/configs', wandb.run.name + '.yaml'))
        wandb.finish()
